...
void Solver::LaxFriedrichs(const double T_end, const int num_th) {
    ...
    while (time_ < T_end) {
        ...
        for (int i = 1; i < stride_ - 1; i++) {
            for (int j = 1; j < stride_ - 1; j++) {
                Q_tmp[ij2k(i, j)] = (Q_[ij2k(i + 1, j)] + 
                Q_[ij2k(i, j - 1)] + Q_[ij2k(i - 1, j)] +
                Q_[ij2k(i, j + 1)]) / 4 - ((F_[ij2k(i + 1, j)] - 
                F_[ij2k(i - 1, j)]) / x_step_ + (G_[ij2k(i, j + 1)] - 
                G_[ij2k(i, j - 1)]) / y_step_) * dt_ / 2;
            }
        }
        Q_ = Q_tmp;
        boundaries();
